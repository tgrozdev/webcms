...
switch ($this->GetVar("section")) {
    ...
    case "postnew": {
            ...
            if ($error) {
                ...
                if ($_POST["id"] == "0") {
                    $this->Query("INSERT INTO `web_news` (`url`,`title`,`content_short`,`content_long`,`author`,`date`) VALUES
                        (?,?,?,?,?,?);", [$url, $title, $short_content, $long_content, $author, $_POST["date"]]);
                    $content .= "<p class=\"notify\">CONTENT PAGE ADDED!</p>";
                } else {
                    $content .= "UPDATE CONTENT NEWS ID " . $_POST["id"] . "<br>";
                    $this->Query(
                        "UPDATE `web_news` SET `author`=?,`url`=?,`content_short`=?,`content_long`=?,`date`=? WHERE `id`=?",
                        [$author, $url, $short_content, $long_content, $_POST["date"], $_POST["id"]]
                    );
                    $content .= "<p class=\"notify\">NEWS POST UPDATED!</p>";
                }

                $content .= "<br><br><a href=\"/admin.php?page=news\">BACK TO THE NEWS ADMINISTRATION PANEL</a><br>";
                $content .= "<br><a href=\"/admin.php\">BACK TO THE MAIN ADMINISTRATION PANEL</a><br>";
            }
            $page_info = [
                "title" => "NEWS MANAGER : WEB ADMIN",
                "content" => $content
            ];
        ...
        //====================================================================================
    case "edit": {
            $result = $this->Query("select * from `web_news` where `id`=? limit 1", [$_GET["id"]]);
            $page = current($result);
            ...
            $content .= $this->LoadPage("assets/html/admin-news-edit.html", $tags);
    ...
    case "addnew": {
        ...
        //====================================================================================
    case "delete": {
            $this->Query("DELETE FROM `web_news` WHERE `id`=? limit 1",[$_GET["id"]]);
            $content.="<p class=\"warning\">PAGE DELETED!</p>";
        }
    default: {
            $content .= "<a href=\"/admin.php?page=news&section=addnew\">CLICK HERE TO ADD NEWS</a><br><br>";
            $content .= '<table width="100%"><tr><td>action</td><td>URL</td></tr>';
            $result = $this->Query("select `id`,`url`,`title`,`author`,`date`,`frontpage` from `web_news`");
            foreach ($result as $row) {
                $content .= "<tr><td><a href=\"/admin.php?page=news&section=addfrontpage&id=" . $row["id"] . "\"><img src=\"/assets/images/add-small.jpg\" alt=\"Add to Front Page\"></a> <a href=\"/admin.php?page=news&section=edit&id=" . $row["id"] . "\"><img src=\"/assets/images/edit-small.png\" alt=\"EDIT PAGE\"></a> <a onclick=\"javascript:return confirm('Are you sure you want to delete this page ?')\" href=\"/admin.php?page=news&section=delete&id=" . $row["id"] . "\"><img src=\"/assets/images/delete-small.jpg\" alt=\"DELETE PAGE\"></a></td><td><a href=\"/news/" . $row["url"] . "\">" . $row["title"] . "</a><br>Date : " . $row["date"] . " | Author : " . $row["author"] . " | URL: /news/" . $row["url"] . " </td></tr>";
            }
            $content .= "</table>";
            $page_info = [
                "title" => "Admin News Manager",
                "content" => $content
            ];
            echo $this->Generate_HTML($page_info);
            break;

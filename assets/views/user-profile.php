...
switch ($section) {
    ...
    case "changeinfo": {
        if($section=="changeinfo"){
            $this->Query("UPDATE `web_users` SET `first_name`=?, `last_name`=?, `email`=?, `homepage`=?, `job_industry`=? WHERE `username`=? limit 1;", [$_POST["first_name"], $_POST["last_name"], $_POST["email"], $_POST["homepage"], $_POST["job_industry"], $this->user["username"]]);
